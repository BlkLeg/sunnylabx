# 🏗️ SunnyLabX Dual Proxmox Architecture Chart (Optimized)
...
│  │  │ • Cloudflare    │ │ • Grafana       │ │ • Home Assistant│                   │   │
│  │  │                 │ │ • Loki          │ │ • MQTT Broker   │                   │   │
│  │  │                 │ │ • Promtail      │ │ • InfluxDB      │                   │   │
│  │  │                 │ │ • AlertManager  │ │                 │                   │   │
│  │  │                 │ │ • Node-Exporter │ │                 │                   │   │
│  │  └─────────────────┘ └─────────────────┘ └─────────────────┘                   │   │
│  │                                                                                 │   │
│  │                   Total: 12 Services (Automation Enhanced)                    │   │
│  └─────────────────────────────────────────────────────────────────────────────────┘   │
│                                                                                         │
│  ┌─────────────────────────────────────────────────────────────────────────────────┐   │
│  │                        Available Resources                                      │   │
...
║  🎯 Resource Efficiency                                                             ║
║  ┌────────────────────────────────────────────────────────────────────────────┐     ║
║  │ • Eliminated Services: Sonarr, Radarr, Overseerr, qBittorrent             │     ║
║  │ • Service Optimization: Single download client (Deluge)                    │     ║
║  │ • Mutual Exclusivity: Plex/Jellyfin saves 1-2GB during normal operation   │     ║
║  │ • Total RAM Liberation: ~4GB freed for application workloads              │     ║
║  │ • Service Count: Streamlined from 46+ → 31 for core functionality         │     ║
║  └────────────────────────────────────────────────────────────────────────────┘     ║
║                                                                                      ║
...
This dual Proxmox cluster represents a **production-ready homelab** with enterprise-grade security, optimal resource utilization, and streamlined service deployment using **Debian containers**. The architecture provides secure network isolation via OPNsense with NordVPN integration and efficient service distribution focused on core functionality.

**Total Infrastructure**: 31 services across 2 nodes with optimized resource allocation and enhanced security posture using VM and LXC isolation.