...
│                                                                                         │
│  ┌─────────────────────────────────────────────────────────────────────────────────┐   │
│  │                   Ubuntu LXC Container (CT 102)                                 │   │
│  │                     6GB RAM, 2 CPU, 13GB SWAP                                  │   │
│  │                       IP: 10.0.0.252/24                                        │   │
│  ├─────────────────────────────────────────────────────────────────────────────────┤   │
│  │                                                                                 │   │
│  │  ┌─────────────────┐ ┌─────────────────┐ ┌─────────────────┐                   │   │
...
│  │  │      (3)        │ │       (6)       │ │       (4)       │                   │   │
│  │  ├─────────────────┤ ├─────────────────┤ ├─────────────────┤                   │   │
│  │  │ • Nginx Proxy   │ │ • Prometheus    │ │ • Matrix        │                   │   │
│  │  │ • Cloudflare    │ │ • Grafana       │ │ • Discord Bot   │                   │   │
│  │  │ • Traefik       │ │ • Loki          │ │ • SMTP Relay    │                   │   │
│  │  │                 │ │ • Promtail      │ │ • Webhooks      │                   │   │
│  │  │                 │ │ • AlertManager  │ │                 │                   │   │
│  │  │                 │ │ • Node-Exporter │ │                 │                   │   │
│  │  └─────────────────┘ └─────────────────┘ └─────────────────┘                   │   │
...
│  ┌─────────────────────────────────────────────────────────────────────────────────┐   │
│  │                        Available Resources                                      │   │
│  │  • 7GB RAM Available for Future Expansion                                      │   │
...
```
    ...
    └─────────┘   └───────────┘  │     │     │  └─────────────┘ └─────────┘
         ...
         │                             │                           │
    ┌────▼────┐   ┌──────▼──────┐      │        ┌─────────▼─────────┐
    │IoT/Auto │   │ Development │      │        │  Communication   │
    │   (7)   │   │     (2)     │      │        │       (4)        │
    └─────────┘   └─────────────┘      │        └───────────────────┘
                                       │
         ┌─────────────────────────────┘
...
This dual Proxmox cluster represents a **production-ready homelab** with enterprise-grade security, optimal resource utilization, and advanced containerization practices. The architecture successfully eliminates redundant services, implements comprehensive security monitoring through Dockerized Wazuh, and provides secure network isolation via OPNsense with NordVPN integration.

**Total Infrastructure**: 52+ services across 2 nodes with 5.75GB RAM liberation and enhanced security posture.